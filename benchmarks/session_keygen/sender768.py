...
class QuantumSecureSender:
    def __init__(self, receiver_ip, port=5000, iterations=100, warmups=20):
    ...
    def connect_to_receiver(self):
        ...
        for i in range(self.warmups):
            ...
            if not start_msg.startswith("START_WARMUP:"):
                self.log(f"Unexpected message from server during warmup: {start_msg}")
                break
        ...
        benchmark_msg = self.socket.recv(1024).decode('utf-8')
        if benchmark_msg != "START_BENCHMARK":
            self.log(f"Unexpected message before benchmark: {benchmark_msg}")
            self.socket.close()
            return

        self.socket.sendall("READY_FOR_BENCHMARK".encode('utf-8'))
    ...
    def run_single_iteration(self, is_warmup=False):
        try:
            ...
            if not self.mutual_authenticate_client(is_warmup):
                return False
    ...
    def mutual_authenticate_client(self, is_warmup=False):
        try:
            if not is_warmup:
...
if __name__ == "__main__":
    ...
    sender = QuantumSecureSender(receiver_ip=args.receiver_ip, port=args.port, iterations=args.iterations, warmups=args.warmups)
