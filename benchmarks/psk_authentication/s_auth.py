...
async def run_authentication_round(receiver_ip, port, psk, round_num):
    ...
    try:
        ...
        if conn_response.get('status') != 'connected':
            print(f"Connection failed: {conn_response.get('reason', 'unknown')}")
            writer.close()
        ...
        if auth_response.get('status') == 'authenticated':
            ...
            print(f"Authentication failed: {auth_response.get('reason', 'unknown')}")
            return False
