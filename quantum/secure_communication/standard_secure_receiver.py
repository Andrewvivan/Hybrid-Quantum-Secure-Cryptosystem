...
class QuantumSecureReceiver:
    def __init__(self):
        self.window = tk.Tk()
        self.window.title("Standard Quantum Secure Communication - Receiver")
        ...
        self.keys_incorrect = False  # Flag if keys do not match
    ...
    def perform_ephemeral_key_exchange(self):
        try:
            ...
            # Compute local key hash
            local_hash = hashlib.sha3_512(self.symmetric_key).hexdigest()
            # Receive sender's key hash
            key_hash_msg = self.client_socket.recv(4096).decode('utf-8').strip()
            if not key_hash_msg.startswith("KEY_HASH:"):
                messagebox.showerror("Ephemeral Key Exchange Error", "Missing key hash from sender")
                self.client_socket.close()
                return False
            sender_hash = key_hash_msg[len("KEY_HASH:"):]
            # Compare hashes
            ...
            try:
                ...
                if self.recv_buffer.startswith("FILE:"):
                    ...
                    try:
                        ...
                        is_decrypted = True
                        # If decryption succeeded, ask whether to save the file
                        ...
                        # When decryption fails, ask only once if keys are mismatched
                        ...
                        if save_raw_response:
                            ...
                            continue
                        # If keys are not mismatched, ask an additional save prompt
