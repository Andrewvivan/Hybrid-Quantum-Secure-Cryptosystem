...
import csv
import os.path
from kyber_wrapper512 import generate_keypair, decapsulate
...
class QuantumSecureReceiver:
    ...
    def mutual_authenticate_server(self):
        ...
        try:
            ...
            server_challenge_b64 = base64.b64encode(server_challenge).decode('utf-8')
            self.client_socket.sendall(("SERVER_CHALLENGE:" + server_challenge_b64).encode('utf-8'))
            ...
            expected_hmac = hmac.new(self.symmetric_key, server_challenge, hashlib.sha256).hexdigest()
            ...
            client_challenge_b64 = base64.b64encode(client_challenge).decode('utf-8')
            self.client_socket.sendall(("CLIENT_CHALLENGE:" + client_challenge_b64).encode('utf-8'))
            server_response = self.client_socket.recv(2048).decode('utf-8')
            if not server_response.startswith("SERVER_RESPONSE:"):
                messagebox.showerror("Authentication Error", "Invalid response from client (server challenge)")
                self.client_socket.close()
                return False
            server_hmac = server_response[len("SERVER_RESPONSE:"):]
            expected_server_hmac = hmac.new(self.symmetric_key, client_challenge, hashlib.sha256).hexdigest()
