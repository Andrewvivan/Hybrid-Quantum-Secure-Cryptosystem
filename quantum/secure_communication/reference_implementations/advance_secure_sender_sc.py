...
class QuantumSecureSender:
    ...
    def setup_chat_frame(self):
        ...
        ttk.Button(button_frame, text="Save Performance Logs", command=self.save_performance_logs).pack(side='left', padx=5)
        ...
        try:
            ...
            if response == "KEY_MISMATCH_REJECTED":
                ...
                if sender_decision:
                    ...
                    return
            elif response.startswith("SERVER_CHALLENGE:"):
    ...
    def mutual_authenticate_client(self):
        if self.show_encrypted:
            return True
        try:
            self.display_message("Authenticaing please wait...")
            server_challenge_msg = self.first_msg if hasattr(self, 'first_msg') else self.socket.recv(2048).decode('utf-8')
            if not server_challenge_msg.startswith("SERVER_CHALLENGE:"):
                messagebox.showerror("Authentication Error", "Invalid challenge from server")
                self.socket.close()
                return False
            server_challenge_b64 = server_challenge_msg[len("SERVER_CHALLENGE:"):]
            server_challenge = base64.b64decode(server_challenge_b64)
            client_hmac = hmac.new(self.symmetric_key, server_challenge, hashlib.sha256).hexdigest()
            self.socket.sendall(("CLIENT_RESPONSE:" + client_hmac).encode('utf-8'))
            client_challenge_msg = self.socket.recv(2048).decode('utf-8')
            if not client_challenge_msg.startswith("CLIENT_CHALLENGE:"):
                messagebox.showerror("Authentication Error", "Invalid client challenge from server")
                self.socket.close()
                return False
            client_challenge_b64 = client_challenge_msg[len("CLIENT_CHALLENGE:"):]
            client_challenge = base64.b64decode(client_challenge_b64)
            server_response = hmac.new(self.symmetric_key, client_challenge, hashlib.sha256).hexdigest()
            self.socket.sendall(("SERVER_RESPONSE:" + server_response).encode('utf-8'))
