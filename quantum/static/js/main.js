...
function setupRectangleInteractions() {
    const rectItems = document.querySelectorAll('.rect-item');
    
    rectItems.forEach(item => {
        item.addEventListener('mouseenter', () => {
            const content = item.querySelector('.rect-content');
            content.style.transform = 'translateZ(20px)';
            content.style.boxShadow = '0 0 30px rgba(0, 210, 255, 0.8)';
        });
        
        item.addEventListener('mouseleave', () => {
            const content = item.querySelector('.rect-content');
            content.style.transform = 'translateZ(0)';
            content.style.boxShadow = '0 0 15px rgba(0, 210, 255, 0.5)';
        });
    });
}
